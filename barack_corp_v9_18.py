...
def msg_intro_groupe(nom_tontine: str, montant: int,
    ...
    return (
        ...
        f"─────────────────────────────────────────────\n\n"
        f"▪ *Retard > 72h* → Suspension automatique + pénalité de réactivation : 1 000 FCFA\n"
    ...
    if tontine:
        ...
        if tontine_en_cours and participants:
            ...
            _wa_send_groupe(group_id,
                ...
                f"   *Conformité COBAC R-2019/01 | ANIF | Archivage 7 ans*\n"
                f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
                f"Ce groupe est désormais administré par *TontineBot Pro*.\n"
                f"*{inscrits} membre(s) enregistré(s) automatiquement* — aucun frais requis pour ce cycle.\n\n"
                f"📌 *CE QUI CHANGE DÈS MAINTENANT*\n"
                f"─────────────────────────────────────────────\n\n"
                f"▪ Chaque screenshot est analysé par *empreinte SHA-256*.\n"
                f"  Un screenshot recyclé, modifié ou falsifié est détecté *immédiatement*.\n\n"
                f"▪ Votre *identifiant WhatsApp, adresse IP et métadonnées de session* "
                f"sont enregistrés et archivés *7 ans*, communicables sur réquisition "
                f"de la *Police Judiciaire, du Parquet ou de l'ANIF*.\n\n"
                f"▪ Tout comportement est analysé en temps réel par notre "
                f"*Trust Graph comportemental*. Un score de risque est calculé "
                f"sur chaque membre — invisible, permanent, irréversible.\n\n"
                f"📌 *SANCTIONS AUTOMATIQUES — AUCUNE DÉCISION HUMAINE REQUISE*\n"
                f"─────────────────────────────────────────────\n\n"
                f"▪ *Retard > 72h* → Suspension + 1 000 FCFA de réactivation\n"
                f"▪ *Paiement après heure limite* → Reclassement en dernière position\n"
                f"▪ *Abandon après bouffage* → Saisie caution + dossier transmis à la Police Judiciaire\n"
                f"▪ *Screenshot falsifié* → Exclusion définitive + poursuites pénales\n"
                f"▪ *3 anomalies* → Bannissement réseau BADF + signalement opérateurs Mobile Money\n\n"
                f"*Ces sanctions se déclenchent seules. "
                f"Elles ne peuvent pas être annulées après déclenchement.*\n\n"
                f"Base légale : Articles 318–323 Code Pénal Camerounais — "
                f"jusqu'à *10 ans d'emprisonnement ferme*.\n\n"
                f"📋 *Vérification d'identité (KYC) demandée à chacun en fin de cycle.*\n\n"
                f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
                f"_Barack & AI Development Facilities Ltd — BADF Ltd_\n"
                f"_« Utiliser la technologie pour servir le prochain avec intégrité »_"
            )
        ...
        for t in tontines:
            ...
            for m in en_retard:
                q(conn, """UPDATE membres
                           ...
                           WHERE id=%s""", (m["id"],))
                _update_score_confiance(conn, m["id"], delta=-25, raison="Suspension retard paiement")
                q(conn, "UPDATE adhesions SET statut='Suspendu' WHERE membre_id=%s AND tontine_id=%s",
                  (m["id"], t["id"]))
                q(conn, """INSERT INTO sanctions
                           (membre_id, tontine_id, type_sanction, notes)
                           VALUES (%s,%s,'Suspension_72h','Suspension auto 72h sans cotisation')""",
                  (m["id"], t["id"]))
                conn.commit()
                wa_prive(m["whatsapp"],
                    f"🔴 *COMPTE SUSPENDU — BARACK CORP*\n"
                    f"Tontine *{t['nom']}*\n"
                    f"Vous n'avez pas cotisé depuis *72h*.\n\n"
                    f"Pour vous réactiver, payez *{FRAIS_REACTIV:,} FCFA*\n"
                    f"Code : *REACTIV*\n"
                    f"MTN : *✱126✱1000✱REACTIV#*\n"
                    f"Orange : *#150✱1000✱REACTIV#*\n\n"
                    f"⚠️ Sans régularisation sous 48h, votre caution sera saisie."
                )
                # Annonce publique dans le groupe — pression sociale
                if t.get("whatsapp_groupe"):
                    wa_groupe(t["whatsapp_groupe"],
                        f"🔴 *{t['nom']} — SUSPENSION*\n\n"
                        f"Le compte de *{m['nom_complet']}* a été suspendu "
                        f"pour non-paiement depuis 72h.\n\n"
                        f"Son accès au menu est bloqué jusqu'à régularisation.\n"
